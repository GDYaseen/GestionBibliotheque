...
public class BorrowDAO {
    ...
    public List<Borrow> getAllBorrows() {
        ...
        String query = "SELECT b.id, s.id AS student_id, s.name AS student_name, "
                     + "bk.id AS book_id, bk.title, bk.author, bk.isbn, bk.published_year, bk.is_available,"
        ...
        try (Connection connection = DbConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                book.setPublisher(rs.getString("isbn"));
    ...
    public String save(Borrow borrow) {
        System.out.println("Borrow : "+borrow.getId()+"   "+borrow.getStudent()+"    "+borrow.getBook());
        if (borrow.getId() > 0) {
    ...
    public String updateBorrow(Borrow borrow) {
        String query = "UPDATE borrows SET student_id = ?, book_id = ?, borrow_date = ? WHERE id = ?";
        try (Connection connection = DbConnection.getConnection();
             PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setInt(1, borrow.getStudent());
            stmt.setInt(2, borrow.getBook());
            stmt.setDate(3, new java.sql.Date(borrow.getBorrowDate().getTime()));
            stmt.setInt(4, borrow.getId());
            stmt.executeUpdate();
